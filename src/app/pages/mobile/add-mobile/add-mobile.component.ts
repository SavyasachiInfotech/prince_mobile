...
export class AddMobileComponent implements OnInit {
  ...
  setMobiles(){
    this._mobileService.getMobiles(0).subscribe(res=>{
      //@ts-ignore
      if(res.status==200){
        //@ts-ignore
  ...
  addMobile(){
    if(this.mobile.name!=""){
      if(this.editBit){
        this._mobileService.updateMobile(this.mobile).subscribe(res=>{
        ...
        this._mobileService.addMobile(this.mobile).subscribe(res=>{
