...
export class AddAttributeValueComponent implements OnInit {
  ...
  constructor(private _attributeValueService:AttributeValueService,
  ...
  setAttributes(){
    this._attributeService.getAttributes(0).subscribe(res=>{
      //@ts-ignore
      if(res.status==200){
        //@ts-ignore
    ...
    this._attributeValueService.getAttributes(0,1).subscribe(
      res=>{
        //@ts-ignore
        if(res.status==200){
          //@ts-ignore
  ...
  changeAttribute(){
    this._attributeValueService.getAttributes(0,this.attribute.attribute_id).subscribe(
      res=>{
        //@ts-ignore
        if(res.status==200){
          //@ts-ignore
  ...
  cancelAttribute(){
    this.editBit=false;
    this.attribute={attribute_id:1,value:""};
  }
  ...
  saveAttribute(){
    if(this.attribute.value==""){
      ...
      if(this.editBit){
        this._attributeValueService.editAttribute(this.attribute,this.attribute.id).subscribe(
          res=>{
            //@ts-ignore
            if(res.status==200){
